...
function cargoMenusEncabezado() {
    ...
    for (var i_1 = 0; i_1 < 3; i_1++) {
        select2.append("<option value=" + 0 + ">" + Clases.tipoNac[0] + "</option>");
    }
    mostrarCiudad(i);
...
function mostrarCiudad(i) {
    var select3 = $("#tipoCiu");
    select3.append("<option value=" + i + ">" + Clases.tipoCiu[0] + "</option>").show();
    //// select3.append("<option value="+1+">"+Clases.tipoCiu[1]+"</option>").hide();
}
var tablaAux;
...
function tablaDinamica(checkboxON) {
    ...
    if (checkboxON.length != 0) {
        ...
        for (var i = 0; i < ciclo; i++) {
            ...
            checkboxON.includes("NAC") == true ? tablaTXT.append("<td id='tipoNac" + i + "'>" + Clases.tipoNac[$('#tipoNac' + i)[0].innerHTML] + "</td>") : null;
            checkboxON.includes("CIU") == true ? tablaTXT.append("<td id='tipoCiu" + i + "'>" + Clases.tipoCiu[$('#tipoCiu' + i)[0].innerHTML] + "</td>") : null;
    ...
    for (var i = 0; i < EmpleadosString.length; i++) {
        ...
        if (empleadoActual != null) {
            ...
            var varAppend = "<tr><td id='mascID" + i + "'>" + empleadoActual._id + "</td>" +
                ...
                "<td id='tipoNac" + i + "'>" + Clases.tipoNac[empleadoActual.pais] + "</td>" +
                "<td id='tipoCiu" + i + "'>" + Clases.tipoCiu[empleadoActual.ciudad] + "</td>" +
...
function agregarEmpleado() {
    ...
    var tipoN = Number($('#tipoNac').val());
    var tipoC = Number($('#tipoNac').val());
    ;
    //Creo el nuevo empleado con los datos del form.
    var nuevoEmpleado = new Clases.Empleado(String($('#nombre').val()), Number($('#edad').val()), String($('#sexo').val()), tipo, tipoN, tipoC, imagenBASE64);
...
function modificarEmpleado(indice) {
    ...
    tcuerpo[0].innerHTML =
        " <div class'row'>" +
            ...
            "</div>" +
            "<div class='form-group'>" +
            "<label for='opcion'>Elige nacionalidad:</label>" +
            "<select class='form-control' name='tipoNac' id='tipoNac'>" +
            "</select>" +
            "</div>" +
            "<div class='form-group'>" +
            "<label for='opcion'>Ciudad:</label>" +
            "<select class='form-control' name='tipoCiu' id='tipoCiu'>" +
            "</select>" +
            "</div>" +
            "<div class='form-group'>" +
